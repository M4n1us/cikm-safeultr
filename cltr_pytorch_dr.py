...
from src.utils.click_trainer import (trainer, trainer_dr, trainer_regression,
                                     trainer_risk)
...
def main():
    ...
    if click_model_type == 'pbm':
        ...
        print("alpha, betas are")
        print(alpha)
        print(beta)
        # try dr with misspecified alpha, beta
        # alpha, beta = trust_bias_misspec(max_cand_size, k)
        # print("alpha beta with error are")
        # print(alpha)
        # print(beta)
        alpha_exp = alpha
    ...
    if args.risk == 1:
        ...
        logging_model = trainer_dr(num_queries=args.num_sessions, num_samples=config['ranking']['num_samples'], k=k,
                                   lr=lr_risk, optimizer=optimizer, alpha_w=alpha, beta_w=beta,
                                   train_qid_map=None, num_docs=max_cand_size,
                                   meta_dir=meta_dir,  device=device,
                                   wandb=wandb, risk_file=results_file_risk,
