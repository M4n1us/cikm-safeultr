...
from src.utils.click_trainer import (trainer, trainer_dr_risk,
...
def main():
    ...
    train_ltr_dataloader = ClickLogDataloader(meta_dir=meta_dir, click_dir=predict_dir, mode='train', feat_vec_dim=doc_feat_size,
                                              max_cand_size=max_cand_size, clip=clipping_val, click_model=click_model_type, estimator='ips')  # , clip_alpha=clipping_val)
    ...
    val_ltr_dataloader = ClickLogDataloader(meta_dir=meta_dir, click_dir=predict_dir, mode='val',
                                            feat_vec_dim=doc_feat_size, max_cand_size=max_cand_size, clip=0., click_model=click_model_type)
    ...
    if args.risk == 1:
        results_file_risk = open(os.path.join('./results/test_results', args.dataset + '_' + str(
            args.T) + '_' + str(args.num_sessions) + '_' + str(args.job_id) + '_' + 'risk_exp'), 'w')
