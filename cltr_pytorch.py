...
import wandb
from src.utils.click_trainer import trainer, trainer_risk, trainer_dr_risk, trainer_regression
...
def main():
    ...
    train_ltr_dataloader = ClickLogDataloader(meta_dir=meta_dir, click_dir=predict_dir, mode='train', feat_vec_dim=doc_feat_size, max_cand_size=max_cand_size, clip=clipping_val, click_model=click_model_type, estimator='ips')#, clip_alpha=clipping_val)
    ...
    val_ltr_dataloader = ClickLogDataloader(meta_dir=meta_dir, click_dir=predict_dir, mode='val', feat_vec_dim=doc_feat_size, max_cand_size=max_cand_size, clip=0., click_model=click_model_type)
    ...
    if args.risk == 1:
        ...
        logging_model = trainer_dr_risk(num_queries=args.num_sessions, num_samples=config['ranking']['num_samples'], k=k, \
                            lr=lr_risk, optimizer=optimizer, alpha_w=alpha, beta_w=beta, \
                                train_qid_map=None, num_docs=max_cand_size,\
                            meta_dir = meta_dir,  device=device, \
                                wandb=wandb, risk_file=results_file_risk,
