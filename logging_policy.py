...
from src.models.PLRanker import PlackettLuceModel, PLRanker
...
wandb.login()
wandb.init(project=(args.dataset + str(args.noise)), entity="yamisuki-tu-wien")
wandb.run.name = args.dataset + \
    str(args.noise) + "Logging_sessions" + str(args.num_sessions)
wandb.run.save()
...
def main():
    ...
    train_ltr_dataloader = LTRDataLoader(qrel_path=train_svmlight, train=True, normalize=query_normalize, k=k, noise=float(
        args.noise), max_cand_size=max_cand_size, meta_dir=meta_dir, mode='train', save=True)
    # rel_scale = 0.25
    rel_scale = 0.25
    ...
    if query_normalize:
        ...
        scaler = None
    val_ltr_dataloader = LTRDataLoader(qrel_path=val_svmlight, train=False, normalize=query_normalize, scaler=scaler, k=k, noise=float(
        args.noise), max_cand_size=max_cand_size, meta_dir=meta_dir, mode='val', save=True)
    if args.dataset == 'MQ2007':
        rel_scale = 0.5
        # rel_scale = 0.025
    else:
    ...
    test_ltr_dataloader = LTRDataLoader(qrel_path=test_svmlight, train=False, normalize=query_normalize, scaler=scaler, k=k, noise=float(
    ...
    logging_ltr_dataloader = LTRLoggerDataLoader(
        train_df_logging, k=k, train_frac=args.fraction)
    ...
    click_sim_dl = LTRClickDataLoader(
        num_sessions=num_sessions, train_df=train_df_click, k=k)
    ...
    click_sim_dl_val = LTRClickDataLoader(
        num_sessions=num_sessions_val, train_df=val_df, k=k)
    # val_ltr_dataloader.set_label(rel_scale)
    click_sim_dataloader_val = DataLoader(
        click_sim_dl_val, batch_size=batch_size, shuffle=False, num_workers=4, persistent_workers=True)
    ...
    if not deterministic:
        ...
        doc_prob_rank_freq = (logging_model.doc_per_rank_prob_det.sum(-1)/(
            (logging_model.query_freq).unsqueeze(1))).nan_to_num_().detach().numpy()
        exp_beta_freq = (logging_model.doc_per_rank_prob_beta.sum(-1) /
                         ((logging_model.query_freq).unsqueeze(1))).nan_to_num_().detach().numpy()
        exp_alpha = (doc_prob_rank * alpha[:k].reshape(1, 1, k)).sum(-1)
        exp_beta = (doc_prob_rank * beta[:k].reshape(1, 1, k)).sum(-1)
        exp_alpha_beta = (
            doc_prob_rank * (beta[:k].reshape(1, 1, k) + alpha[:k].reshape(1, 1, k))).sum(-1)
        ...
        display_mask = logging_model.doc_per_rank_prob_det.sum(
            -1).to(bool).to(torch.int)
    else:
    ...
    qid_ctr_map = (logging_model.qid_clicks.sum(-1) /
                   ((logging_model.query_freq).unsqueeze(1))).nan_to_num_().detach().numpy()
    ...
    query_train, query_val = train_test_split(
        active_queries, test_size=0.15, random_state=42)
